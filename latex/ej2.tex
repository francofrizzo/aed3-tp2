\section{Ejercicio 2: El Imperio contraataca}
    ...
    \renewcommand\constante{25500000}
    ...
    Por último, en las figuras \ref{fig:exp2:cociente1} y \ref{fig:exp2:cociente2} se muestra el cociente entre los datos obtenidos y las funciones $M$ y $M \times log(N)$ respectivamente. 

    \begin{figure}[H]
        \centering
        \caption{}
        \label{fig:exp2:cociente1}
        \begin{tikzpicture}
            \begin{axis}[
                    title={},
                    xlabel={Cantidad de aristas($M$)},
                ...
                \addplot[color=black] table[x index=0,y expr={\thisrowno{1} / x}]{../exp/elImperioContraatacaArbolesMod};
                \addplot[color=red] table[x index=0, y expr={ln(x + 1)*\constante}]{../exp/elImperioContraatacaArbolesMod};
                \legend{$\frac{T}{M}$, $c*log(N)$}
            \end{axis}
        \end{tikzpicture}
    \end{figure}

    Se debe observar que es posible utilizar $N$ para el cociente debido a que los experimentos se realizan con árboles, por lo que variar $M$ repercute en $N$ siempre de la misma forma.

    \begin{figure}[H]
        \centering
        \caption{}
        \label{fig:exp2:cociente2}
        \begin{tikzpicture}
            \begin{axis}[
                    title={},
                    xlabel={Cantidad de aristas($M$)},
                ...
                \addplot[color=black] table[x index=0,y expr={\thisrowno{1} / (x * ln(x + 1))}]{../exp/elImperioContraatacaArbolesMod};
