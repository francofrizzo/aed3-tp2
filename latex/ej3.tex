\section{Ejercicio 3: El retorno del jedi}
    ...
    Calcular el salto vertical u horizontal tiene un costo de $\ord(1)$. Calcular la primera fila tiene costo $\ord(M)$, ya que se debe recorrer la primera fila y almacenar en la matriz de subsoluciones ($DP$), la suma de la anterior más el salto horizontal. Calcular la primera la primera columna cuesta $\ord(N)$, ya que se debe recorrer la primera columna e ir guardando en la matriz $DP$, para cada elemento, la suma del resultado anterior más el costo del salto vertical.
        ...
        \begin{tikzpicture}
            \begin{axis}[
               ...
               \addplot[color=red] table[x index=0, y expr={125}]{../exp/elRetornoDelJediFilas};
                ...
                \legend{$T_M/M$,$T_N/N$,$c$}
            \end{axis}
        \end{tikzpicture}
    \end{figure}

    En el gráfico de la figura \ref{fig:exp3:var-nym-divconst}, se puede ver que si se lo divide por $N$ o $M$, el tiempo de ejecución queda acotable por una constante, lo cual confirma la linealidad de $T_N$ y $T_M$.
    \begin{figure}[H]
        \centering
        \caption{}
    ...
    En la Figura \ref{fig:exp3:var-nxn-base} se puede ver que al variar el $N$ (y simultáneamente el $M$, ya que se considera $M = N$), $T_{M \times N}$ se puede acotar por una función cuadrática.
    \begin{figure}[H]
        \centering
        \caption{}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \legend{$T_{M \times N}/(N*N)$, $c*N*N$}
            \end{axis}
        \end{tikzpicture}
    \end{figure}

    En el gráfico de la Figura \ref{fig:exp3:var-nxn-divconst} se puede ver que al dividir por $N \times N$, queda acotable por una constante, lo cual confirma que $T_{M \times N}$ tiene complejidad $O(M \times N)$.
    ...
    En el gráfico de la Figura \ref{fig:exp3:varh}, por último, se puede ver que $T_H$ es acotable por por una constante, lo cual corrobora que variar el $H$ no afecta la complejidad del algoritmo.
